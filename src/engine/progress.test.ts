...
describe('evidence-based progress reports',()=>{
 ...
 expect(r.readinessScore).toBe('75%');expect(r.strengths).toEqual(['Routing: 3/3 correct']);expect(r.criticalVulnerabilities).toEqual(['Review BOVPN']);expect(r.recommendedLabs).toEqual([watchguardLabs.find(l=>l.id===15)!.name]);
 });
