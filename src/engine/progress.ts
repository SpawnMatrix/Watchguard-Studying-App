...
export function buildStudyReport(input:any={}) {
  ...
  // Weakest first, so a caller showing only the first few recommendations shows the ones that matter most.
  const weaknesses=history?[...topics].filter(([,s])=>s.correct<s.total).sort(([,a],[,b])=>a.correct/a.total-b.correct/b.total||b.total-a.total).map(([topic])=>topic):(Array.isArray(input?.topicWeaknesses)?input.topicWeaknesses.filter((v:any)=>typeof v==='string'):[]);
  ...
  const ids=[...new Set<number>(weaknesses.flatMap((topic:string)=>labTopics[topic]??[]))];
  return {
