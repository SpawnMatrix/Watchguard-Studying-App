...
const LOCAL_QA_DATABASE: QAItem[] = [
  {
    ...
    refLink:
      "https://www.watchguard.com/help/docs/help-center/en-US/Content/en-US/Fireware/initial_setup/factory_default_settings_c.html",
  },
  {
    id: 2,
    category: "Setup",
  ...
  },
  {
    ...
    category: "Policies",
    question:
      "What is the difference between Packet Filters and Proxy Policies?",
    answer:
      "• **Packet Filters**: Operate at Layer 3 (Network) and Layer 4 (Transport). They inspect only IP headers, port numbers, and protocol codes. They are extremely fast but completely blind to the actual content/data passing through.\n• **Proxies (Application Layer Gateways)**: Operate at Layer 7 (Application). They intercept the connection, terminate the TCP handshake, reconstruct and parse the payload, enforce strict RFC protocol standards, and can inspect MIME types, block malicious patterns, or scan files.",
    keywords: [
      "packet filter",
      "proxy",
      "layer 7",
  ...
  },
  {
    ...
    refLink:
      "https://www.watchguard.com/help/docs/help-center/en-US/Content/en-US/Fireware/proxies/https/https_proxy_contentinspect_about_c.html",
  },
  {
    id: 6,
    category: "Routing",
    ...
    answer:
      "NAT Loopback (also called Hairpin NAT) allows internal clients on Trusted or Optional interfaces to connect to a public-facing server (e.g., an internal web server mapped via Static NAT) using its **public IP address or public domain name** rather than its internal private IP.\n\nWithout NAT Loopback, internal requests to the public IP would be dropped because the Firebox external interface doesn't route packets that originate from and terminate in the same internal subnets.",
    keywords: [
      "nat loopback",
      "hairpin",
      "snat",
  ...
  },
  {
    ...
    refLink:
      "https://www.watchguard.com/help/docs/help-center/en-US/Content/en-US/Fireware/nat/nat_1to1_about_c.html",
  },
  {
    id: 8,
    category: "VPN",
    ...
    answer:
      "• **Phase 1 (Gateway)**: Negotiates the primary security handshake between the two physical Firebox endpoints. It authenticates peers using a **Pre-Shared Key** or certificate, selects Phase 1 encryption/hashing (AES/SHA), and defines **Gateway IDs**. If Phase 1 fails, the tunnel cannot start.\n• **Phase 2 (Tunnel)**: Negotiates the specific secure tunnel parameters for routing traffic between subnets. It defines matching **local and remote network ranges** (tunnel routes) and IPSec protocols (ESP/AH) with Perfect Forward Secrecy (PFS) settings.",
    keywords: [
      "bovpn",
      "phase 1",
      "phase 2",
  ...
  },
  {
    ...
    question:
      "How long is an IP address blocked by default on the Blocked Sites list, and how does it work?",
    answer:
      "When default threat protection or IPS detects an attack, the offending source IP address is dynamically added to the **Temporary Blocked Sites** list.\n\n• **Default Duration**: **20 minutes**.\n• This duration is fully customizable by administrators under Global settings. \n• Administrators can also manually add IPs permanently under the **Permanent Blocked Sites** tab in Policy Manager.",
    keywords: [
      "blocked sites",
      "blocked",
      "temporary",
  ...
  },
  {
    ...
    refLink:
      "https://www.watchguard.com/help/docs/help-center/en-US/Content/en-US/Fireware/fsm/diagnostic_tasks_policy_check_fsm_c.html",
  },
  {
    id: 11,
    category: "Routing",
    ...
    refLink:
      "https://www.watchguard.com/help/docs/help-center/en-US/Content/en-US/Fireware/routing/dynamic_routing_about_c.html",
  },
  {
    id: 12,
    category: "Routing",
  ...
  },
  {
    ...
    refLink:
      "https://www.watchguard.com/help/docs/help-center/en-US/Content/en-US/Fireware/mvpn/mvpn_comparison_c.html",
  },
  {
    id: 15,
    category: "VPN",
    question:
      "What is the structural difference between Virtual Interface BOVPN and Policy-Based BOVPN?",
    answer:
      "• **Policy-Based BOVPN**: Binds secure communication strictly to static policies. Traffic is defined by explicit local/remote IP pairs matching the gateway selectors. Perfect Forward Secrecy (PFS) is applied directly on the policy action.\n• **Virtual Interface BOVPN**: Creates a logical virtual tunnel interface (e.g., `bvpn1`). Because it behaves like a standard physical interface, you can route traffic over it using static routes, dynamic routing (OSPF), or SD-WAN. This is much more flexible and is the modern standard for hub-and-spoke mesh topologies.",
    keywords: [
      "virtual interface",
      "bovpn",
      "policy-based",
  ...
  },
  {
    ...
    refLink:
      "https://www.watchguard.com/help/docs/help-center/en-US/Content/en-US/Fireware/services/apt/apt_blocker_about_c.html",
  },
  {
    id: 18,
    category: "Policies",
  ...
  },
  {
    ...
    refLink:
      "https://www.watchguard.com/help/docs/help-center/en-US/Content/en-US/Fireware/fsm/diagnostic_tasks_fsm_c.html",
  },
];
...
export default function GeneralChat() {
  ...
  const [isAIFeaturesEnabledState, setIsAIFeaturesEnabledState] =
    useState(false);
  const [_globalAIEnabled, setGlobalAIEnabled] = useState(false);
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <div className="flex items-center space-x-1.5">
              <span
                className={`w-1.5 h-1.5 rounded-full ${isAIFeaturesEnabledState ? "bg-green-500 animate-ping" : "bg-gray-500"}`}
              ></span>
              <span className="text-[10px] font-mono text-gray-400">
                {isAIFeaturesEnabledState
                  ? "AI Tutor Mode Authorized"
                  : "Syllabus Reference mode"}
              </span>
            </div>
          </div>
        </div>
          ...
          <button
            onClick={() => {
              if (!isAIFeaturesEnabledState) {
                alert(
                  "AI features are currently toggled offline by the administrator or require your own override key in the Admin Panel.",
                );
              }
              setMode("ai");
            }}
          ...
          >
            <AnimatePresence initial={false}>
              {messages.map((msg) => (
                ...
                >
                  <div
                    className={`flex space-x-3 max-w-[85%] ${msg.sender === "user" ? "flex-row-reverse space-x-reverse" : "flex-row"}`}
                  >
                    {/* Avatar */}
                    <div
                    ...
                    <div className="space-y-3">
                      <div
                        className={`px-4 py-3.5 rounded-xl border text-sm leading-relaxed ${
                          msg.sender === "user"
                      ...
                      >
                          ...
                          {msg.text.split("\n").map((line, idx) => {
                            if (
                              ...
                              return (
                                <div
                                  key={idx}
                                  className="pl-4 my-1 flex items-start space-x-2"
                                >
                                  <span className="text-watchguard-orange mt-1.5 text-[8px]">
                                    ■
                                  </span>
                            ...
                            if (/^\d+\s*\.\s/.test(line)) {
                              ...
                              return (
                                <div
                                  ...
                                  className="pl-4 my-1 flex items-start space-x-2"
                                >
                                  <span className="text-watchguard-orange font-mono font-medium">
                                    {num}.
                                  </span>
                                  <span>
                                    {parseBold(line.substring(num.length + 2))}
                                  </span>
                                </div>
                              );
                            }
                        ...
                        {msg.isDemo && (
                          <div className="mt-3 pt-2 border-t border-watchguard-border flex items-center space-x-2 text-[10px] font-mono text-gray-400">
                            ...
                            <span>
                              Demo Offline Mode • Fallback Answers Loaded
                            </span>
                          </div>
                        )}
                      </div>
                      ...
                      {msg.sender === "bot" &&
                        msg.requiresExternalLookup &&
                        msg.suggestedSearchTerms && (
                          <motion.div
                          ...
                          >
                            ...
                            <p className="text-xs text-gray-400 leading-normal">
                              This query involves dynamic or obscure
                              configuration rules. For verified specifications,
                              search the official WatchGuard documentation:
                            </p>
                            <a
                            ...
                            >
                              <span>
                                Search Guide: "{msg.suggestedSearchTerms}"
                              </span>
              ...
              <div className="flex justify-start">
                <div className="flex space-x-3 max-w-[85%]">
                  ...
                  <div className="px-4 py-3 bg-watchguard-lightgray text-gray-400 border border-watchguard-border rounded-xl text-sm flex items-center space-x-2">
                    <span className="animate-pulse-soft font-mono text-xs">
                      Formulating click-by-click topology mapping...
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="flex-1 flex flex-col bg-watchguard-dark/30 p-6 space-y-6 overflow-y-auto">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="font-display font-semibold text-white">
                Verified Local Q&A Database
              </h3>
            </div>
            <p className="text-xs text-gray-400 leading-relaxed font-sans">
              AI features are offline by default to conserve your server quota.
              Browse and query the curated local knowledge base. It contains
              verified, click-by-click configuration specifications directly
              matching your network security essentials syllabus.
            </p>
          </div>
            ...
            {filteredQA.length === 0 ? (
              <div className="bg-watchguard-gray border border-watchguard-border rounded-xl p-8 text-center space-y-2">
                ...
                <p className="text-sm text-gray-400 font-mono">
                  No matching syllabus Q&A items found.
                </p>
                <button
                  onClick={() => {
                                ...
                                if (
                                  ...
                                  return (
                                    <div
                                      ...
                                      className="pl-4 my-1 flex items-start space-x-2"
                                    >
                                      <span className="text-watchguard-orange mt-1.5 text-[6px]">
                                        ■
                                      </span>
                                      <span>
                            ...
                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between border-t border-watchguard-border/30 pt-3.5 gap-2 text-[11px]">
                              ...
                              <div className="flex flex-wrap gap-1.5 items-center">
                                ...
                                {item.keywords.slice(0, 4).map((word, wIdx) => (
                                  <span
                                    key={wIdx}
                                    className="bg-watchguard-dark/60 px-1.5 py-0.5 border border-watchguard-border/40 rounded text-gray-400 font-mono text-[10px]"
                                  >
                                    {word}
                                  </span>
                                ))}
          ...
          <div className="bg-watchguard-orange/5 border border-watchguard-orange/20 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <span className="text-[10px] font-mono text-watchguard-orange uppercase tracking-wider font-bold">
                Online Resource Center
              </span>
              <p className="text-xs text-gray-300 font-sans leading-normal">
                Want to expand your knowledge with official WatchGuard videos,
                guides, and Quizlet study sheets? Open the{" "}
                <strong>Flashcard Studio</strong> tab to find active links and
                resources!
              </p>
            </div>
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith("`") && part.endsWith("`")) {
      return (
        <code
          key={i}
          className="px-1.5 py-0.5 bg-watchguard-dark border border-watchguard-border rounded text-xs text-watchguard-orange font-mono"
        >
          {part.slice(1, -1)}
        </code>
      );
