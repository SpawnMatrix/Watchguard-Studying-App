...
function getAIClient(customApiKey?: string): GoogleGenAI {
  ...
  if (
    ...
    throw new Error(
      "AI features are disabled or GEMINI_API_KEY is not configured.",
    );
  }
        ...
        responseSchema: {
          ...
          properties: {
            message: {
              type: Type.STRING,
              description:
                "The detailed conversational reply with markdown formatting, WSM/Web UI click paths, and supportive tutor tone.",
            },
            requiresExternalLookup: {
              type: Type.BOOLEAN,
              description:
                "Set to true if query deals with obscure or changing third party integrations.",
            },
            suggestedSearchTerms: {
              type: Type.STRING,
              description:
                "Search terms for the WatchGuard Help Center if requiresExternalLookup is true.",
            },
          },
        ...
        responseSchema: {
          ...
          properties: {
            ...
            detailedExplanation: {
              type: Type.STRING,
              description:
                "Technical explanation of the correct choice and why distractors fail, referencing WatchGuard guidelines.",
            },
            weaknessCategory: {
              type: Type.STRING,
              description:
                "Must be: 'NAT', 'Mobile VPN', 'BOVPN', 'Routing', 'Policies', 'Proxies', 'Security Services', 'Initial Setup', 'Logging & Monitoring'.",
            },
          },
...
Technician described issue: "${technicianIssue}"`,
        ...
        responseSchema: {
          ...
          properties: {
            analysis: {
              type: Type.STRING,
              description:
                "Step-by-step diagnostic analysis of the routing or proxy failure.",
            },
            suggestedCommand: {
              type: Type.STRING,
              description:
                "Concrete command to run, e.g. ping command, policy query, etc.",
            },
            simulatedLogs: {
              type: Type.ARRAY,
              items: { type: Type.STRING },
              description:
                "2-4 simulated line logs from FSM Traffic Monitor, TCP Dump, or Policy Checker showing the failure.",
            },
          },
    ...
    handleError(
      "AI Service diagnoseLabFailure failed, using local fallback",
      err,
    );
    return getLocalDiagnosticsFallback(labName, stepTitle, technicianIssue);
  }
 ...
 */
export async function analyzeCertificationPerformance(
  sessionHistory: any,
  customApiKey?: string,
): Promise<CertificationPerformanceReport> {
  if (!isAIFeaturesEnabled(customApiKey)) {
  ...
  try {
    ...
    handleError(
      "AI Service analyzeCertificationPerformance failed, using local fallback",
      err,
    );
    return getLocalPerformanceFallback(sessionHistory);
  }
...
4. **NAT Loopback:**
  ...
  if (
    p.includes("proxy") ||
    p.includes("webblocker") ||
    p.includes("inspection") ||
    p.includes("body content")
  ) {
    return {
...
function getLocalQuizFallback(
  ...
  if (questionId !== undefined && selectedOptions !== undefined) {
    q = examQuestionsMap.get(questionId);
    if (q) {
      topic = q.topic;
      const correctList = q.correctAnswers;
      if (q.isMultiSelect) {
        isCorrect =
          selectedOptions.length === correctList.length &&
          selectedOptions.every((ans) => correctList.includes(ans));
      } else {
        isCorrect =
          selectedOptions.length === 1 && selectedOptions[0] === correctList[0];
      }
    }
  }
  ...
  const correctAnswersDisplay =
    questionId !== undefined
      ? (q?.correctAnswers || [correctAnswer]).join(", ")
      : correctAnswer;
...
function getLocalDiagnosticsFallback(
  labName: string,
  stepTitle: string,
  technicianIssue: string,
) {
...
The described problem ("${technicianIssue}") points to a configuration discrepancy in security policies or routing modes.`;
  ...
  if (
    issue.includes("vpn") ||
    issue.includes("bovpn") ||
    issue.includes("tunnel") ||
    issue.includes("phase")
  ) {
...
3. Validate link monitor targets to ensure interfaces aren't marked logically dead.`;
    ...
    issue.includes("proxy") ||
    issue.includes("http") ||
    issue.includes("certificate") ||
    issue.includes("cert")
  ) {
...
3. Alternatively, check **HTTP Response > Body Content Types** in the HTTP proxy action to see if the file extension download (e.g. .pdf or .exe) is set to Deny instead of AV Scan.`;
    suggestedCommand =
      "FSM > Tools > Diagnostic Tasks > TCP Dump -i eth1 port 4126";
    simulatedLogs = [
...
function getLocalPerformanceFallback(sessionHistory: any) {
  ...
  let strengths = [
    ...
    "Default Threat Protection Settings",
  ];
  let criticalVulnerabilities = [
    "BOVPN Mismatched Proposals",
    "HTTPS Proxy Content Inspection Certificates",
  ];
  let recommendedLabs = ["Lab Exercise 11: Proxies", "Lab Exercise 16: BOVPNs"];
  let summary =
    "Local diagnostic telemetry indicates stable fundamentals in initial deployment setup. However, advanced tasks like route failovers, multi-WAN configurations, and Layer 7 deep content filters need immediate study. Practicing the hands-on lab guides will rapidly improve the readiness score.";
  ...
  if (quizAttempts >= 3) {
    if (quizScoreNum >= 75) {
      ...
      strengths = [
        ...
        "Static Routing Mappings",
        "RFC 1918 Private Ranges",
      ];
      criticalVulnerabilities = ["Complex Multi-WAN routing tables"];
      recommendedLabs = ["Lab Exercise 8: Link Monitor and SD-WAN"];
      summary =
        "Excellent comprehension shown. Your quiz accuracy scores exceed the 75% standard goal. Core security zoning, DNS query resolutions, and packet filters are fully understood. Focus on reviewing high-availability SD-WAN multi-WAN routing tables before attempting the official certification.";
    } else {
