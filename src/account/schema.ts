import { validLabProgress } from '../engine/labProgress';

/** Only study data is synchronized. API keys, PINs and browser identity are excluded. */
export const STUDY_KEYS = ['watchguard-study-progress-v1','weakness_deck','watchguard_mastered_flashcards','watchguard-quiz-session-v2','watchguard-study-profile-name-v1','watchguard-srs-v1','watchguard-lab-progress-v1'] as const;
...
export function validStudyValue(key:string,value:string):boolean {
  try {
    ...
    if(key==='watchguard-lab-progress-v1')return validLabProgress(v);
