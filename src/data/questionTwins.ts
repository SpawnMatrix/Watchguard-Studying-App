 ...
 *
 * The bank grew in passes, and some facts were written more than once: the Authentication Portal
 * port, the default Trusted address, what NAT loopback is for. Asked in practice, a second wording is
...
export const QUESTION_TWINS: readonly (readonly number[])[] = [
  ...
  [302, 1188], // Authentication Portal port
  ...
  [55, 1166], // APT Blocker cloud sandbox analysis
  ...
  [67, 126, 1005], // default Web UI URL and port
  [25, 1200, 1840], // IKE on UDP 500 and 4500
  [46, 1201], // Mobile VPN with SSL on TCP 443
  [82, 113, 1207], // SSL VPN pool must not overlap other networks
  ...
  [37, 48, 1002], // Firebox System Manager live status and diagnostics
  [33, 1000], // Policy Manager edits configuration offline
  [58, 93], // Traffic Monitor shows log messages in real time
  [84, 1041, 1537], // historical reports need retained logs
  [9, 80], // the TCP-UDP proxy
  [24, 35, 1526], // DLP needs Gateway AntiVirus and, for HTTPS, content inspection
  [1505, 1530, 1221], // Phase 1 up, Phase 2 fails
  [1012, 1539], // feature keys belong to one device
  [1222, 1603], // BOVPN tunnel routes are mirrored at the peer
  [1046, 1508, 1536, 1609], // an Allow entry does not prove the application worked
  [1510, 1541], // VLAN tagging mismatch with the link up
  [1063, 1542], // missing return route
  [1008, 1538], // backup image before an upgrade
  [1047, 1545], // traffic fields needed to reproduce a policy lookup
  [1068, 1807], // longest prefix match
  [1070, 1808], // the default route
  [1100, 1845], // many clients sharing one public address
  // Network+
  [110, 1305], // /27 mask
