...
export const labCategories = ['Setup & Management','Networking & Routing','Policies & Proxies','Security Services','Authentication','VPN','Logging & Reporting'] as const;
export type LabCategory = typeof labCategories[number];

export interface Lab {
  id: number;
  name: string;
  /** Browsing axis for the catalogue. Independent of the question topics. */
  category: LabCategory;
  /** True when the lab needs more than one Firebox and a management PC:
   *  a second appliance, an AD domain controller, or a Dimension/Cloud subscription. */
  requiresExtraKit: boolean;
  objectives: string;
...
export type LabSort = "number" | "category" | "name";
export interface LabView { sortBy: LabSort; category: "All" | LabCategory; singleFireboxOnly: boolean }

/**
 * Filter and order the catalogue for display. Pure so the browsing rules can be tested without
 * rendering: the catalogue previously shipped out of numeric order because the component rendered
 * the source array directly.
 */
export function orderLabs(labs: readonly Lab[], view: LabView): Lab[] {
  const shown = labs.filter(lab =>
    (view.category === "All" || lab.category === view.category) &&
    (!view.singleFireboxOnly || !lab.requiresExtraKit));
  const ordered = [...shown];
  if (view.sortBy === "name") ordered.sort((a, b) => a.name.localeCompare(b.name));
  else if (view.sortBy === "category") ordered.sort((a, b) => a.category.localeCompare(b.category) || a.id - b.id);
  else ordered.sort((a, b) => a.id - b.id);
  return ordered;
}
