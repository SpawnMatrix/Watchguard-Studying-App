...
- **Interactive NSE labs**: all 20 Lab Book exercises, each step paired with a checkpoint question (81 in total) that must be answered correctly to complete it. Six single-Firebox labs (1, 3, 6, 9, 10, 17) are **hands-on**: they open a simulated Firebox and management PC, and each step checks the configuration you actually made. Progress is saved per step and syncs with your account, so a lab can be finished across sessions, with or without a Firebox in front of you.
