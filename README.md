...
- **Interactive NSE labs**: all 20 Lab Book exercises, each step paired with a checkpoint question (81 in total) that must be answered correctly to complete it. Nine single-Firebox labs (1, 3, 6, 8, 9, 10, 11, 14, 17) are **hands-on**: they open a simulated Firebox and management PC, and each step checks the configuration you actually made. Progress is saved per step and syncs with your account, so a lab can be finished across sessions, with or without a Firebox in front of you.
...
- **Exam readiness** on Progress & Admin: each NSE category or Network+ domain is scored from your latest answer to every question, weighted by the published outline and compared with the NSE 75% pass mark, with the category to study next. Lab Exercises opens on a lab dashboard with a Resume button for the lab in progress.
